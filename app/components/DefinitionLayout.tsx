...
import { DefinitionArticle, definitionArticles } from "../data/definition-articles";
import Link from "next/link";
import ContactSection from "./ContactSection";
import { md } from "../utils/markdown";

export default function DefinitionLayout({ article }: { article: DefinitionArticle }) {
  const related = definitionArticles.filter(a => article.relatedSlugs.includes(a.slug));
  ...
  return (
    <div className="bg-white">
      ...
      <div className="mx-auto max-w-3xl px-6 lg:px-8 py-20">
        ...
        {/* Definition Box */}
        <div className="bg-ebony rounded-[2.5rem] p-8 mb-12">
      ...
      {related.length > 0 && (
        <section className="py-20 bg-gray-50 border-t border-gray-100">
          <div className="mx-auto max-w-3xl px-6 lg:px-8">
            <h2 className="text-2xl font-black text-ebony uppercase tracking-tight mb-10 text-center">Related Health Guides</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {related.map(rel => (
                <Link href={`/resources/${rel.slug}`} key={rel.slug} className="group bg-white p-7 rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all border border-gray-100 block">
                  <span className="text-[10px] font-black text-brand-start uppercase tracking-widest bg-brand-start/10 px-3 py-1 rounded-full mb-5 inline-block">Health Glossary</span>
